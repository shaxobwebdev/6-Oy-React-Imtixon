...
function Header({ headerText, headerLogo, logo, signUp }) {
  const activeLink = " bg-white ";
  const normalLink = "";
  ...
  return (
    <div className=" container w-[100%] mx-auto mt-5  ">
      ...
      <div className="lg:w-[85%] sm:w-[50%] w-full mx-auto flex items-center justify-between mt-5">
        <div className="flex items-center gap-10 lg:w-auto">
          <NavLink to={navs[0].route}>
            <img src={headerLogo} alt={logo} className=" text[50px]" />
          </NavLink>
            ...
            <ul className="flex lg:items-center flex-col lg:flex-row">
              {navs.map((nav) => {
                return (
                  <li
                    key={nav.id}
                    className="cursor-pointer font-[400] text-[16px] hover:bg-[#a2a2b0] px-[24px] rounded-[8px] py-[14px] transition ease-in-out duration-500"
                  >
                    <NavLink
                      to={nav.route}
                      className={({ isActive }) =>
        ...
        <div className="flex items-center gap-3 md:gap-5 justify-end">
          <span className="cursor-pointer font-[400] text-[16px] hover:bg-[#F1F1F3] px-[12px] rounded-[8px] py-[14px] transition  duration-500">
            <NavLink
              to="/Sign Up"
...
function Bars({ navs }) {
  ...
  return (
    <div className={`barsSection lg:hidden`}>
      ...
      />
      <nav
        className={`navLinks duration-500 ease-in-out lg:static absolute bg-[#c2e419] min-h-[40vh] lg:min-h-[0vh] top-[28%] md:top-[16%]  z-[1000] overflow-hidden left-0 w-full flex items-center px-5 shadow-lg ${
          isBarsClicked ? "opacity-100 visible" : "opacity-20 invisible"
        }`}
      >
